...
predicted_trees_sf$match_label <- ifelse(predicted_trees_sf$match, "True Positive", "False Positive")
ground_truth$match_label <- "False Negative"
...
ggplot(combined_data) +
  geom_sf(aes(color = match_label, shape = match_label), size = 3, alpha = 0.7) +
  scale_color_manual(
    name = "Prediction Status",
    values = c("True Positive" = "green", "False Positive" = "red", "False Negative" = "grey30")
  ) +
  scale_shape_manual(
    name = "Prediction Status",
    values = c("True Positive" = 20, "False Positive" = 16, "False Negative" = 4)
  ) +
